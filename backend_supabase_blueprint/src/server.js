...
function requireAuth(roles = []) { return (req, res, next) => { if (hasSupabase) return next(); const user = authUserFromRequest(req); if (!user) return res.status(401).json({ error: 'Нужна авторизация' }); if (roles.length && !roles.includes(user.role)) return res.status(403).json({ error: 'Недостаточно прав' }); req.user = user; next(); }; }
